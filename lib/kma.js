...
export async function fetchShortTerm(lat, lon, targetDateStr) {
  ...
  if (!header || header.resultCode !== '00') {
    throw new Error(`기상청 단기예보 API 오류: ${header ? header.resultMsg : 'unknown'}`);
  }
...
export async function fetchMidTerm(address, targetDateStr) {
  ...
  const url = `https://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getMidLandFcst`
  ...
  const header = json?.response?.header;
  if (!header || header.resultCode !== '00') {
    throw new Error(`기상청 중기예보 API 오류: ${header ? header.resultMsg : 'unknown'}`);
  }
